namespace VRTK.Examples
{
    ...
    public class check2: VRTK_InteractableObject
    {
        ...
        protected override void Update()
        {
            ...
            else
            {
                if (open2 > 0)
                {
                    ...
                    if (open2 == 0)
                    {
                        if ((GameObject.Find("System").transform.localPosition.x) == 15f)
                        {
                            GameObject.Find("Box010").GetComponent<BoxCollider>().enabled = false;
                            GameObject.Find("二级菜单/1menu-1/Text").GetComponent<Text>().text = "模块检查完毕，SIM卡缺失，请更换";
                            image.DOMove(new Vector3(-0.194f, 0.449f, -0.2f), 0.5f);
                            GameObject.Find("System").transform.localPosition = new Vector3(0f, 8f, 0f);                           
                            GameObject.Find("Capsule077").GetComponent<MeshRenderer>().material.color = Color.red;
                            //设置更改窗口的大小
